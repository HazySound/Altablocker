...
- **전역 차단 모드** — 켜져 있는 동안 모든 창에서 차단
- **윈도우 키 차단** — Alt를 누르다 실수로 Win 키를 눌러 시작 메뉴가 열리는 사고도 방지. Alt+Tab과 개별 on/off 가능
- **원클릭 토글** — 스위치를 끄면 그 즉시 완전 원상복구
- **차단 카운터** — 키가 실제로 막힐 때마다 횟수 표시 (동작 확인용)
...
동작은 단순합니다: **"Alt가 눌린 상태에서 들어온 Tab 키 다운"과 (옵션) "윈도우 키 다운" 이벤트를 Windows에 전달하지 않고 무시**합니다. 그 결과 창 전환·시작 메뉴만 발동하지 않습니다.

| 이 프로그램이 하는 것 | 이 프로그램이 **하지 않는** 것 |
|---|---|
| OS 수준에서 Alt+Tab·윈도우 키 입력 무시 | ❌ 키 입력 생성·주입 (`SendInput` 등 일절 없음) |
